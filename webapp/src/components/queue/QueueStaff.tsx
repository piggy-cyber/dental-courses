...
export function QueueStaff({ slug }: { slug: string }) {
  ...
  if (snapshot.lobby.closedAt) return <QueueFrame slug={slug}><section className={styles.panel}><div className={styles.signInBlock}><p className={styles.eyebrow}>Lobby closed</p><h1>{snapshot.lobby.name}</h1><p>This lobby is not accepting new staff candidates or promotion responses.</p><Link className={styles.secondaryButton} href="/queue/dashboard">Back to dashboard</Link></div></section></QueueFrame>;
