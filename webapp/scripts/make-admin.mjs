// Promotes an account to admin (owner role) and approves it.
// Sign in on the site once first, then run:
//   node scripts/make-admin.mjs you@gmail.com
...
if (!email) {
  console.error("Usage: node scripts/make-admin.mjs you@gmail.com");
  process.exit(1);
}
...
console.log(`${email} is now an admin with full access.`);
