revoke insert, update on table public.game_progress from authenticated;
revoke insert on table public.game_rounds from authenticated;

drop policy if exists "Approved students can create their own game progress"
on public.game_progress;
drop policy if exists "Approved students can update their own game progress"
on public.game_progress;
drop policy if exists "Approved students can create their own game round receipts"
on public.game_rounds;

alter table public.game_progress
  add constraint game_progress_best_score_bounded
  check (best_score <= 26000);

alter table public.game_rounds
  drop constraint game_rounds_score_nonnegative,
  drop constraint game_rounds_attempts_valid,
  add constraint game_rounds_score_bounded
    check (score between 0 and 26000),
  add constraint game_rounds_attempts_bounded
    check (attempts between 1 and 100),
  add constraint game_rounds_score_matches_correct
    check (score <= correct * 260);
